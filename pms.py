...
con = sqltor.connect(host="localhost", user="root", password="mozhi@2526")
...
def setup_db():
    ...
    CREATE TABLE IF NOT EXISTS appointments (
        ...
        CONSTRAINT fk_patient FOREIGN KEY (patient_id) REFERENCES appt(idno) ON DELETE CASCADE
    )
    """)
...
def ensure_unique_appt_no(doctor_name, appt_date):
    for _ in range(100):
        ...
        cur.execute("""SELECT 1 FROM appointments 
                       WHERE doctor_name=%s AND appt_date=%s AND appt_no=%s""",
                    (doctor_name, appt_date, num))
        if not cur.fetchone():
            return num
    cur.execute("""SELECT COALESCE(MAX(appt_no), 9) FROM appointments 
                   WHERE doctor_name=%s AND appt_date=%s""", (doctor_name, appt_date))
